...
const getGranularFertilizerSchema = (t: (msg: string) => string) =>
  z.object({
...
type GranularFertilizerFormData = z.infer<
  ReturnType<typeof getGranularFertilizerSchema>
>;

interface AddGranularFertilizerFormProps {
  open: boolean;
...
export function AddGranularFertilizerForm({
  open,
  onOpenChange,
  onSuccess,
  editItemId,
}: AddGranularFertilizerFormProps) {
  ...
  const form = useForm<GranularFertilizerFormData>({
    resolver: zodResolver(getGranularFertilizerSchema(t)),
    defaultValues: {
      name: "",
      contents: "",
      nitrogenContent: 0,
  ...
  const onSubmit = async (data: GranularFertilizerFormData) => {
    if (!convexOrg?._id) {
    ...
    try {
      if (isEditMode && editItemId) {
        ...
        toast.success(i18n._("Granular fertilizer updated successfully"));
      } else {
        await createInventory({
          ...
          contents: data.contents,
          nitrogenContent: data.nitrogenContent,
          fertilizerType: "гранулиран",
        });
        toast.success(i18n._("Granular fertilizer added successfully"));
      }
      form.reset({
        name: "",
        contents: "",
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? (
              <Trans
                id="Edit Granulated Fertilizer"
                message="Редактиране на гранулиран тор"
              />
            ) : (
              <Trans id="Granulated Fertilizer" message="Гранулиран тор" />
            )}
          </DialogTitle>
          <DialogDescription>
            {isEditMode ? (
              <Trans
                id="Update the granulated fertilizer information below."
                message="Обновете информацията за гранулирания тор по-долу."
              />
            ) : (
              <Trans
                id="Fill in the required information to add a granulated fertilizer."
                message="Попълнете необходимата информация за добавяне на гранулиран тор."
              />
            )}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormItem>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Тон">
                        <Trans id="Ton" message="Тон" />
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage />
            ...
            <FormItem>
              ...
              <FormControl>
                <Input value="Гранулиран" disabled readOnly />
              </FormControl>
            </FormItem>
